...
namespace Archon.Extractors.AspNet.Tests
{
    ...
    public sealed class WorkerHostedServiceRuntimeExtractorTests
    {
        ...
        public void Extract_WhenBackgroundServiceExists_ShouldContributeHostedServiceAndExecutionMethodFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "archon-wp008-worker-extractor-" + Guid.NewGuid().ToString("N"));
        ...
        {
            // The prior snapshot mimics WP007 AddHostedService output so Work Item 5 can prove cross-slice correlation without invoking the DI extractor.
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "archon-wp008-worker-extractor-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(Path.Combine(repositoryRoot, "src", "Customer.Worker"));
            try
            {
        ...
        {
            // The edge shape mirrors WP007 REGISTERED_AS_SERVICE metadata fields consumed by worker-hosted-service correlation.
