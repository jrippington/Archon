...
{
    /// <summary>
    /// Verifies the UI/client Razor Pages and MVC Razor extraction stage participates in API-triggered extraction orchestration.
    /// </summary>
    public sealed class RazorPageViewExtractionStageTests
    {
        /// <summary>
        /// Verifies API module registration keeps the UI/client Razor Pages and MVC Razor adapter available to the unified UI/client stage.
        /// </summary>
        [Fact]
        public void AddArchonExtractionApi_ShouldRegisterUiClientRazorStage()
        {
            // The API module exposes one unified UI/client pipeline stage while keeping the Razor adapter resolvable for direct stage tests and unified orchestration.
            ServiceProvider provider = new ServiceCollection()
                .AddArchonExtractionApi()
                .BuildServiceProvider();

            RazorPageViewExtractionStage stage = provider.GetRequiredService<RazorPageViewExtractionStage>();
        ...
        public async Task ExecuteAsync_WhenRazorPageExists_ShouldAccumulateUiFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "ArchonUiClientRazorStageTests", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                RazorPageViewExtractionStage stage = new(new RazorPageViewExtractor(), NullLogger<RazorPageViewExtractionStage>.Instance);
        ...
        /// <summary>
        /// Creates a pipeline stage context suitable for UI/client Razor API stage tests.
        /// </summary>
        ...
        private static ExtractionStageContext CreateContext(string repositoryRoot)
        {
            ...
            ExtractionRun run = new(
                ...
                completedUtc: null,
                new ExtractionRunProgress("Queued", "Queued for UI/client Razor stage test.", 0, new DateTimeOffset(2026, 5, 20, 8, 0, 0, TimeSpan.Zero)),
                warnings: null,
                errors: null,
                timings: null,
