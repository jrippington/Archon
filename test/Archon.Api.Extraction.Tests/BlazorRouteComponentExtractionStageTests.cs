...
{
    /// <summary>
    /// Verifies the UI/client Blazor extraction stage participates in API-triggered extraction orchestration.
    /// </summary>
    public sealed class BlazorRouteComponentExtractionStageTests
    {
        /// <summary>
        /// Verifies API module registration keeps the UI/client Blazor adapter available to the unified UI/client stage.
        /// </summary>
        [Fact]
        public void AddArchonExtractionApi_ShouldRegisterUiClientBlazorStage()
        {
            // The API module exposes one unified UI/client pipeline stage while keeping the Blazor adapter resolvable for direct stage tests and unified orchestration.
            ServiceProvider provider = new ServiceCollection()
                .AddArchonExtractionApi()
                .BuildServiceProvider();

            BlazorRouteComponentExtractionStage stage = provider.GetRequiredService<BlazorRouteComponentExtractionStage>();
        ...
        public async Task ExecuteAsync_WhenRazorComponentExists_ShouldAccumulateUiFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "ArchonUiClientStageTests", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                BlazorRouteComponentExtractionStage stage = new(new BlazorRouteComponentExtractor(), NullLogger<BlazorRouteComponentExtractionStage>.Instance);
        ...
        /// <summary>
        /// Creates a pipeline stage context suitable for UI/client API stage tests.
        /// </summary>
        ...
        private static ExtractionStageContext CreateContext(string repositoryRoot)
        {
            ...
            ExtractionRun run = new(
                ...
                completedUtc: null,
                new ExtractionRunProgress("Queued", "Queued for UI/client stage test.", 0, new DateTimeOffset(2026, 5, 20, 8, 0, 0, TimeSpan.Zero)),
                warnings: null,
                errors: null,
                timings: null,
