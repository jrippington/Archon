...
{
    /// <summary>
    /// Verifies the WP011 Avalonia AXAML extraction stage participates in API-triggered extraction orchestration.
    /// </summary>
    public sealed class Wp011AvaloniaAxamlExtractionStageTests
    {
        /// <summary>
        /// Verifies API module registration keeps the WP011 Avalonia AXAML adapter available to the unified UI/client stage.
        /// </summary>
        [Fact]
        public void AddArchonExtractionApi_ShouldRegisterWp011AvaloniaStage()
        {
            // The API module exposes one unified WP011 pipeline stage while keeping the Avalonia adapter resolvable for direct stage tests and unified orchestration.
            ServiceProvider provider = new ServiceCollection()
                .AddArchonExtractionApi()
                .BuildServiceProvider();

            Wp011AvaloniaAxamlExtractionStage stage = provider.GetRequiredService<Wp011AvaloniaAxamlExtractionStage>();
        ...
        public async Task ExecuteAsync_WhenAvaloniaProjectExists_ShouldAccumulateUiFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "ArchonWp011AvaloniaStageTests", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                Wp011AvaloniaAxamlExtractionStage stage = new(new AvaloniaAxamlExtractor(), NullLogger<Wp011AvaloniaAxamlExtractionStage>.Instance);
        ...
        /// <summary>
        /// Creates a pipeline stage context suitable for WP011 Avalonia API stage tests.
        /// </summary>
        ...
        private static ExtractionStageContext CreateContext(string repositoryRoot)
        {
            ...
            ExtractionRun run = new(
                ...
                completedUtc: null,
                new ExtractionRunProgress("Queued", "Queued for WP011 Avalonia stage test.", 0, new DateTimeOffset(2026, 5, 20, 8, 0, 0, TimeSpan.Zero)),
                warnings: null,
                errors: null,
                timings: null,
