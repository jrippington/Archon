...
{
    /// <summary>
    /// Verifies unified UI/client extraction stage registration and mixed-framework API orchestration behavior.
    /// </summary>
    public sealed class UiClientExtractionStageTests
    {
        /// <summary>
        /// Verifies the API module exposes one unified UI/client pipeline stage while framework-specific adapters remain concrete services.
        /// </summary>
        [Fact]
        public void AddArchonExtractionApi_ShouldRegisterUnifiedUiClientStageOnlyAsPipelineStage()
        {
            // The unified stage is the only IExtractionStage entry for UI/client so API-triggered orchestration sees one UI/client slice after external integration.
            ServiceProvider provider = new ServiceCollection()
                .AddArchonExtractionApi()
                .BuildServiceProvider();

            IExtractionStage[] stages = provider.GetServices<IExtractionStage>().ToArray();
            string[] uiClientStageIds = stages.Select(stage => stage.StageId).Where(stageId => stageId.StartsWith("wp011-", StringComparison.Ordinal)).ToArray();

            Assert.Contains(stages, stage => stage is UiClientExtractionStage);
            Assert.Equal(["wp011-ui-client"], uiClientStageIds);
            Assert.NotNull(provider.GetRequiredService<BlazorRouteComponentExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<RazorPageViewExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<WinFormsStaticUiExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<WpfXamlExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<WinUiXamlExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<MauiXamlExtractionStage>());
            Assert.NotNull(provider.GetRequiredService<AvaloniaAxamlExtractionStage>());
        }

        /// <summary>
        /// Verifies unified UI/client extraction accumulates mixed UI frameworks, redacts evidence, preserves unknowns and warnings, and deduplicates repeated stable-key facts.
        /// </summary>
        ...
        public async Task ExecuteAsync_WhenMixedUiRepositoryExists_ShouldAccumulateDeduplicatedSnapshotFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "ArchonUiClientUnifiedStageTests", Guid.NewGuid().ToString("N"));
        ...
        /// <summary>
        /// Creates a mixed UI repository fixture containing representative artifacts for every UI/client framework extractor.
        /// </summary>
        ...
        /// <summary>
        /// Creates a pipeline stage context suitable for unified UI/client API stage tests.
        /// </summary>
        ...
        private static ExtractionStageContext CreateContext(string repositoryRoot)
        {
            ...
            ExtractionRun run = new(
                ...
                completedUtc: null,
                new ExtractionRunProgress("Queued", "Queued for unified UI/client UI/client stage test.", 0, new DateTimeOffset(2026, 5, 20, 8, 0, 0, TimeSpan.Zero)),
                warnings: null,
                errors: null,
                timings: null,
