...
{
    /// <summary>
    /// Verifies the UI/client Windows Forms static UI extraction stage participates in API-triggered extraction orchestration.
    /// </summary>
    public sealed class WinFormsStaticUiExtractionStageTests
    {
        /// <summary>
        /// Verifies API module registration keeps the UI/client Windows Forms static UI adapter available to the unified UI/client stage.
        /// </summary>
        [Fact]
        public void AddArchonExtractionApi_ShouldRegisterUiClientWinFormsStage()
        {
            // The API module exposes one unified UI/client pipeline stage while keeping the Windows Forms adapter resolvable for direct stage tests and unified orchestration.
            ServiceProvider provider = new ServiceCollection()
                .AddArchonExtractionApi()
                .BuildServiceProvider();

            WinFormsStaticUiExtractionStage stage = provider.GetRequiredService<WinFormsStaticUiExtractionStage>();
        ...
        public async Task ExecuteAsync_WhenWinFormsProjectExists_ShouldAccumulateUiFacts()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "ArchonUiClientWinFormsStageTests", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                WinFormsStaticUiExtractionStage stage = new(new WinFormsStaticUiExtractor(), NullLogger<WinFormsStaticUiExtractionStage>.Instance);
        ...
        /// <summary>
        /// Creates a pipeline stage context suitable for UI/client Windows Forms API stage tests.
        /// </summary>
        ...
        private static ExtractionStageContext CreateContext(string repositoryRoot)
        {
            ...
            ExtractionRun run = new(
                ...
                completedUtc: null,
                new ExtractionRunProgress("Queued", "Queued for UI/client Windows Forms stage test.", 0, new DateTimeOffset(2026, 5, 20, 8, 0, 0, TimeSpan.Zero)),
                warnings: null,
                errors: null,
                timings: null,
