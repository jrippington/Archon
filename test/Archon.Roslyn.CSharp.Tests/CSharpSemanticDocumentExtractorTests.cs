...
namespace Archon.Roslyn.CSharp.Tests
{
    ...
    public sealed class CSharpSemanticDocumentExtractorTests
    {
        ...
        /// <summary>
        /// Confirms that C# extraction preserves resolvable facts while recording diagnostics and unknowns for degraded compilations.
        /// </summary>
        [Fact]
        public void ExtractCapturesDiagnosticsAndUnknownsForDegradedCompilation()
        {
            // The fixture intentionally mixes resolvable declarations with missing types, dynamic dispatch, ambiguous overloads, and reflection so partial extraction remains useful.
            string source = """
                using System;
                ...
                namespace Sample.App
                {
                    public sealed class Worker
                    {
                        public void Run(MissingService missing, dynamic target)
                        {
                            target.Execute();
                            MissingStatic.Execute();
                            string typeName = "Sample.App.Worker";
                ...
                """;
            string repositoryRoot = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "archon-csharp-degraded"));
            string documentPath = Path.Combine(repositoryRoot, "src", "Sample.App", "Worker.cs");
            ...
            Assert.Contains(result.Diagnostics, diagnostic => diagnostic.DiagnosticId.StartsWith("CS", StringComparison.Ordinal) && diagnostic.Severity == SemanticDiagnosticSeverity.Error);
            ...
            Assert.Contains(result.Unknowns, unknown => unknown.Reason == SemanticUnknownReason.DynamicDispatch);
            Assert.Contains(result.Unknowns, unknown => unknown.Reason == SemanticUnknownReason.ReflectionTarget);
            Assert.Contains(result.Unknowns, unknown => unknown.Reason == SemanticUnknownReason.AmbiguousOverload);
            Assert.All(result.Unknowns, unknown => Assert.Equal(SemanticFactConfidence.Unresolved, unknown.Confidence));
        }

        /// <summary>
        /// Confirms that generated partial C# declarations merge into stable identities while preserving generated evidence contribution metadata.
        /// </summary>
        [Fact]
        public void ExtractMarksGeneratedPartialDeclarationsAndPreservesEvidenceContributions()
        {
            // A generated file should not create duplicate type identities for partial classes, but its source span must remain visible as a contribution.
            string source = """
                // <auto-generated />
                namespace Sample.App
                {
                    public partial class GeneratedWidget
                    {
                    ...
                    public partial class GeneratedWidget
                    {
                        public void ManualPart()
                        {
                        }
                    }
                }
                """;
            string repositoryRoot = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "archon-csharp-generated"));
            string documentPath = Path.Combine(repositoryRoot, "src", "Sample.App", "GeneratedWidget.g.cs");
        ...
        /// <summary>
        /// Confirms that C# dependencies on metadata-only framework symbols are classified without inventing repository source declaration nodes.
        /// </summary>
        [Fact]
        public void ExtractClassifiesMetadataSymbolDependencies()
        {
            ...
            string source = """
                ...
                """;
            string repositoryRoot = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "archon-csharp-metadata"));
            string documentPath = Path.Combine(repositoryRoot, "src", "Sample.App", "Worker.cs");
            ...
            Assert.Contains(
                ...
                relationship => relationship.RelationshipKind == SemanticRelationshipKind.DependsOn
                    ...
                    && relationship.TargetSymbolIdentity?.FullyQualifiedName.Contains("DateTime", StringComparison.Ordinal) == true);
