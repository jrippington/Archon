...
namespace Archon.Extractors.Tests.Projects.Solutions
{
    /// <summary>
    /// Verifies the repository and solution extraction stage contributes the first real graph facts through the shared pipeline contract.
    /// </summary>
    public sealed class RepositorySolutionExtractionStageTests : IDisposable
    {
        ...
        {
            // This scenario is the minimal project-extraction scenario: one validated repository root and one explicit solution path.
        ...
        {
            // The extractor must respect the repository validation boundary and only read solution paths that were explicitly submitted.
            string repositoryRoot = CreateRepositoryRoot();
        ...
        private string CreateRepositoryRoot()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "archon-project-extraction-" + Guid.NewGuid().ToString("N"));
        ...
        /// <returns>A resolved extraction input mirroring the repository validation output.</returns>
