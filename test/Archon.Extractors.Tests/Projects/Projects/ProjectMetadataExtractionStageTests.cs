...
namespace Archon.Extractors.Tests.Projects.Projects
{
    /// <summary>
    /// Verifies the project metadata slice extracts C# and VB.NET project nodes through the shared repository/solution stage.
    /// </summary>
    public sealed class ProjectMetadataExtractionStageTests : IDisposable
    {
        ...
        /// <summary>
        /// Verifies a representative repository produces the complete project extraction snapshot shape in one coherent run.
        /// </summary>
        ...
        [Fact]
        public async Task ExecuteAsync_WhenRepresentativeRepositoryIsSubmitted_ShouldProduceCompleteProjectExtractionSnapshot()
        {
        ...
        {
            // Project metadata extraction may safely inspect small repository-contained source artifacts for strong textual indicators without performing Roslyn semantic analysis.
            string repositoryRoot = CreateRepositoryRoot();
        ...
        {
            // A web SDK combined with explicit test project packages is contradictory for project classification, so Unknown is safer than guessing.
            string repositoryRoot = CreateRepositoryRoot();
        ...
        private string CreateRepositoryRoot()
        {
            ...
            string repositoryRoot = Path.Combine(Path.GetTempPath(), "archon-project-metadata-" + Guid.NewGuid().ToString("N"));
        ...
        {
            // The input mirrors repository validation output while avoiding the HTTP layer in project extractor tests.
