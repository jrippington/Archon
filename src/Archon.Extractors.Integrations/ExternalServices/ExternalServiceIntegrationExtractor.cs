...
namespace Archon.Extractors.Integrations.ExternalServices
{
    ...
    public sealed class ExternalServiceIntegrationExtractor
    {
        ...
        {
            // Follow the WP010 detector pattern: collect safe observations first, then reuse the foundation projector for graph consistency.
        ...
        private static void AddUnknownWarning(List<string> warnings, IUnknownDescriptor descriptor, SemanticExtractionRequest semanticDocument, SyntaxNode node)
        {
            ...
            if (!string.IsNullOrWhiteSpace(descriptor.UnknownReason))
            {
                ...
                warnings.Add($"WP010 external-service extraction recorded unresolved target evidence in {Path.GetRelativePath(semanticDocument.RepositoryRootDirectory, semanticDocument.DocumentPath)} at line {span.StartLinePosition.Line + 1}: {descriptor.UnknownReason}");
            }
        }
        ...
        private sealed class ExternalServiceContext
        {
            ...
            {
                // The current context does not need to pre-scan the tree, but the full signature mirrors other WP010 detectors for future extension.
                _ = semanticDocument;
                _ = observations;
                _ = warnings;
                cancellationToken.ThrowIfCancellationRequested();
        ...
        private sealed class ExternalServiceArtifactIndex
        {
            ...
            public static ExternalServiceArtifactIndex Create(string repositoryRootDirectory, List<string> warnings, CancellationToken cancellationToken)
            {
                ...
                {
                    warnings.Add($"WP010 external-service extraction could not scan a configuration artifact: {ex.GetType().Name}.");
                }
