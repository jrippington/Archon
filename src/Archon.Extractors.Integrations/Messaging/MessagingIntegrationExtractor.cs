...
namespace Archon.Extractors.Integrations.Messaging
{
    ...
    public sealed class MessagingIntegrationExtractor
    {
        ...
        {
            // Extraction mirrors other WP010 slices: gather deterministic observations first, then use the foundation projector for graph shape consistency.
        ...
        /// <summary>
        /// Creates semicolon-delimited role metadata understood by the WP010 foundation projection path.
        /// </summary>
        ...
        private static void AddUnknownWarning(List<string> warnings, MessagingTargetDescriptor target, SemanticExtractionRequest semanticDocument, SyntaxNode syntaxNode)
        {
            ...
            {
                warnings.Add($"WP010 messaging extraction recorded unknown {target.TargetKind} at {FormatLocation(semanticDocument, syntaxNode)} because {target.UnknownReason}");
            }
        }
        ...
        private sealed class MessagingArtifactIndex
        {
            ...
            public static MessagingArtifactIndex Create(string repositoryRootDirectory, List<string> warnings, CancellationToken cancellationToken)
            {
                ...
                {
                    warnings.Add($"WP010 messaging extraction skipped unreadable messaging configuration artifact because {ex.GetType().Name} occurred.");
                }
