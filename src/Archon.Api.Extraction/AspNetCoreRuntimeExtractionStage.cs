...
{
    /// <summary>
    /// Runs the first ASP.NET Core runtime ASP.NET Core minimal API endpoint extraction slice as part of the API-triggered extraction pipeline.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class AspNetCoreRuntimeExtractionStage : IExtractionStage
    {
        /// <summary>
        ...
        /// <summary>
        /// Stores the logger used for credential-safe ASP.NET Core runtime orchestration events.
        /// </summary>
        private readonly ILogger<AspNetCoreRuntimeExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AspNetCoreRuntimeExtractionStage" /> class.
        /// </summary>
        /// <param name="logger">The logger used for start, completion, and degraded extraction messages.</param>
        public AspNetCoreRuntimeExtractionStage(ILogger<AspNetCoreRuntimeExtractionStage> logger)
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AspNetCoreRuntimeExtractionStage" /> class with explicit extractor dependencies.
        /// </summary>
        ...
        public AspNetCoreRuntimeExtractionStage(AspNetCoreMinimalApiEndpointExtractor extractor, ConsoleEntryPointRuntimeExtractor consoleEntryPointExtractor, WorkerHostedServiceRuntimeExtractor workerHostedServiceExtractor, NonHttpRuntimeConsumerExtractor nonHttpRuntimeConsumerExtractor, ILogger<AspNetCoreRuntimeExtractionStage> logger)
        {
        ...
        /// <summary>
        /// Loads submitted C# project documents, runs the ASP.NET Core runtime minimal API extractor, and merges endpoint facts into the shared accumulator.
        /// </summary>
        ...
        /// <returns>A successful stage result when ASP.NET Core runtime endpoint extraction completes or degrades non-fatally.</returns>
        public async Task<ExtractionStageResult> ExecuteAsync(ExtractionStageContext context, CancellationToken cancellationToken)
        {
            // ASP.NET Core runtime extraction is non-blocking for individual solution read failures because earlier stages still own controlled solution validation.
            ArgumentNullException.ThrowIfNull(context);
            cancellationToken.ThrowIfCancellationRequested();

            _logger.LogInformation(
                "Starting ASP.NET Core runtime ASP.NET Core minimal API extraction for run {RunId} with {SolutionCount} submitted solution path(s).",
            ...
            foreach (string solutionPath in context.ResolvedInput.SolutionPaths)
            {
                ...
                catch (Exception exception) when (exception is IOException or UnauthorizedAccessException or System.Xml.XmlException or InvalidDataException)
                {
                    ...
                    context.Accumulation.AddWarning("ASP.NET Core runtime ASP.NET Core minimal API extraction skipped a submitted solution because its C# project context could not be loaded.");
                    _logger.LogWarning(exception, "ASP.NET Core runtime ASP.NET Core minimal API extraction skipped solution context for run {RunId}.", context.Run.RunId.ToString());
                }
            }
            ...
            {
                _logger.LogWarning(
                    "ASP.NET Core runtime ASP.NET Core minimal API extraction completed for run {RunId} with {DegradedSolutionCount} degraded solution load outcome(s).",
                    context.Run.RunId.ToString(),
                    degradedSolutionCount);
            }

            _logger.LogInformation(
                "Completed ASP.NET Core runtime ASP.NET Core minimal API, console entry-point, worker hosted-service, and non-HTTP runtime consumer extraction for run {RunId}; inspected {CSharpDocumentCount} C# semantic document(s) and {RuntimeDocumentCount} runtime semantic document(s).",
                context.Run.RunId.ToString(),
        ...
        /// <summary>
        /// Creates the snapshot stable key used to scope ASP.NET Core runtime graph contributions during API orchestration.
        /// </summary>
        ...
        /// <summary>
        /// Represents the semantic documents loaded from one solution for ASP.NET Core runtime runtime extraction.
        /// </summary>
