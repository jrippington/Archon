...
{
    /// <summary>
    /// Runs the UI/client WPF XAML extraction slice as part of API-triggered extraction orchestration.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class WpfXamlExtractionStage : IExtractionStage
    {
        /// <summary>
        ...
        /// <summary>
        /// Stores the logger used for credential-safe UI/client WPF orchestration diagnostics.
        /// </summary>
        private readonly ILogger<WpfXamlExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="WpfXamlExtractionStage" /> class.
        /// </summary>
        ...
        public WpfXamlExtractionStage(WpfXamlExtractor extractor, ILogger<WpfXamlExtractionStage> logger)
        {
        ...
        {
            // UI/client WPF extraction is non-blocking because absence of WPF artifacts should not prevent other extraction facts from being persisted.
            ArgumentNullException.ThrowIfNull(context);
            cancellationToken.ThrowIfCancellationRequested();

            _logger.LogInformation(
                "Starting UI/client WPF XAML extraction for run {RunId}.",
            ...
            _logger.LogInformation(
                "Completed UI/client WPF XAML extraction for run {RunId}; added {NodeCount} node(s), {EdgeCount} edge(s), {EvidenceCount} evidence record(s), {WarningCount} warning(s), and {ErrorCount} error(s) after stable-key deduplication.",
                context.Run.RunId.ToString(),
