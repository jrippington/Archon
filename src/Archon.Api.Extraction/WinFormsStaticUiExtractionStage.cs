...
{
    /// <summary>
    /// Runs the UI/client Windows Forms static UI extraction slice as part of API-triggered extraction orchestration.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class WinFormsStaticUiExtractionStage : IExtractionStage
    {
        /// <summary>
        ...
        /// <summary>
        /// Stores the logger used for credential-safe UI/client Windows Forms orchestration diagnostics.
        /// </summary>
        private readonly ILogger<WinFormsStaticUiExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="WinFormsStaticUiExtractionStage" /> class.
        /// </summary>
        ...
        public WinFormsStaticUiExtractionStage(WinFormsStaticUiExtractor extractor, ILogger<WinFormsStaticUiExtractionStage> logger)
        {
        ...
        {
            // UI/client Windows Forms extraction is non-blocking because absence of desktop UI artifacts should not prevent other extraction facts from being persisted.
            ArgumentNullException.ThrowIfNull(context);
            cancellationToken.ThrowIfCancellationRequested();

            _logger.LogInformation(
                "Starting UI/client Windows Forms static UI extraction for run {RunId}.",
            ...
            _logger.LogInformation(
                "Completed UI/client Windows Forms static UI extraction for run {RunId}; added {NodeCount} node(s), {EdgeCount} edge(s), {EvidenceCount} evidence record(s), {WarningCount} warning(s), and {ErrorCount} error(s) after stable-key deduplication.",
                context.Run.RunId.ToString(),
