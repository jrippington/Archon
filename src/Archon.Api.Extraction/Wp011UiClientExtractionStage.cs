...
{
    /// <summary>
    /// Coordinates every WP011 .NET UI/client framework extractor through one API-triggered extraction stage.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class Wp011UiClientExtractionStage : IExtractionStage
    {
        /// <summary>
        /// Stores the deterministic framework stage adapters in the order used for unified WP011 extraction.
        /// </summary>
        private readonly IReadOnlyList<IExtractionStage> _frameworkStages;

        /// <summary>
        /// Stores the logger used for credential-safe unified WP011 orchestration diagnostics.
        /// </summary>
        private readonly ILogger<Wp011UiClientExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="Wp011UiClientExtractionStage" /> class.
        /// </summary>
        ...
        public Wp011UiClientExtractionStage(
            Wp011BlazorRouteComponentExtractionStage blazorStage,
            Wp011RazorPageViewExtractionStage razorStage,
            Wp011WinFormsStaticUiExtractionStage winFormsStage,
            Wp011WpfXamlExtractionStage wpfStage,
            Wp011WinUiXamlExtractionStage winUiStage,
            Wp011MauiXamlExtractionStage mauiStage,
            Wp011AvaloniaAxamlExtractionStage avaloniaStage,
            ILogger<Wp011UiClientExtractionStage> logger)
        {
            // The constructor fixes framework ordering once so API composition exposes one stable WP011 stage while preserving each framework adapter's existing behavior.
            _frameworkStages =
            [
        ...
        /// <summary>
        /// Executes every WP011 UI/client framework adapter against the shared extraction accumulation.
        /// </summary>
        ...
        public async Task<ExtractionStageResult> ExecuteAsync(ExtractionStageContext context, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation(
                "Starting unified WP011 UI/client extraction for run {RunId} with {FrameworkStageCount} framework stage(s).",
            ...
            foreach (IExtractionStage frameworkStage in _frameworkStages)
            {
                ...
                _logger.LogInformation(
                    "Starting WP011 framework stage {FrameworkStageId} for run {RunId}.",
                ...
                if (result.HasBlockingError)
                {
                    ...
                    _logger.LogWarning(
                        "WP011 framework stage {FrameworkStageId} stopped unified UI/client extraction for run {RunId} with a controlled blocking error.",
                        frameworkStage.StageId,
                        context.Run.RunId.ToString());
                    return result;
                }
                ...
                _logger.LogInformation(
                    "Completed WP011 framework stage {FrameworkStageId} for run {RunId}; added {NodeDelta} node(s), {EdgeDelta} edge(s), {EvidenceDelta} evidence record(s), {WarningDelta} warning(s), and {ErrorDelta} error(s) after stable-key deduplication.",
                    frameworkStage.StageId,
                    context.Run.RunId.ToString(),
            ...
            _logger.LogInformation(
                "Completed unified WP011 UI/client extraction for run {RunId}; accumulated {NodeCount} node(s), {EdgeCount} edge(s), {EvidenceCount} evidence record(s), {WarningCount} warning(s), and {ErrorCount} error(s).",
                context.Run.RunId.ToString(),
