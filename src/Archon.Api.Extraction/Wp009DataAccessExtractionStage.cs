...
{
    /// <summary>
    /// Runs the WP009 data-access extraction vertical slice as part of the API-triggered extraction pipeline.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public sealed class Wp009DataAccessExtractionStage : IExtractionStage
    {
        /// <summary>
        ...
        /// <summary>
        /// Stores the logger used for credential-safe WP009 orchestration events.
        /// </summary>
        private readonly ILogger<Wp009DataAccessExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="Wp009DataAccessExtractionStage" /> class.
        /// </summary>
        /// <param name="logger">The logger used for start, completion, and degraded extraction messages.</param>
        public Wp009DataAccessExtractionStage(ILogger<Wp009DataAccessExtractionStage> logger)
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Wp009DataAccessExtractionStage" /> class with an explicit extractor dependency.
        /// </summary>
        ...
        public Wp009DataAccessExtractionStage(LinqToSqlDbmlModelExtractor dbmlExtractor, ILogger<Wp009DataAccessExtractionStage> logger)
        {
        ...
        {
            // WP009 DBML extraction is non-blocking because malformed files and unreadable roots are represented as diagnostics or empty contributions.
            ArgumentNullException.ThrowIfNull(context);
            cancellationToken.ThrowIfCancellationRequested();

            _logger.LogInformation(
                "Starting WP009 data-access DBML extraction for run {RunId}.",
            ...
            _logger.LogInformation(
                "Completed WP009 data-access DBML extraction for run {RunId}; contributed {NodeCount} node(s), {EdgeCount} edge(s), and {EvidenceCount} evidence record(s).",
                context.Run.RunId.ToString(),
        ...
        /// <summary>
        /// Loads Roslyn semantic documents for WP009 generated designer and source-usage extraction.
        /// </summary>
        ...
        {
            // WP009 reuses the lightweight Roslyn loader from the semantic stage so it can inspect generated designer and source usage without adding MSBuildWorkspace coupling.
            ...
            foreach (string solutionPath in context.ResolvedInput.SolutionPaths)
            {
                ...
                {
                    context.Accumulation.AddWarning($"WP009 data-access source usage extraction skipped solution '{Path.GetFileName(solutionPath)}' because semantic documents could not be loaded.");
                    _logger.LogWarning(exception, "WP009 data-access source usage extraction skipped semantic loading for solution {SolutionPath} during run {RunId}.", solutionPath, context.Run.RunId.ToString());
                }
            }
