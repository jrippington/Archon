...
{
    /// <summary>
    /// Runs the WP011 WPF XAML extraction slice as part of API-triggered extraction orchestration.
    /// </summary>
    /// <remarks>
    /// The stage is an orchestration adapter only. It delegates static project, XAML, resource, and code-behind analysis to <see cref="WpfXamlExtractor" /> and merges the returned graph facts into the shared accumulator without compiling WPF projects, loading XAML, starting dispatchers, connecting to databases, or writing directly to Neo4j.
    /// </remarks>
    public sealed class Wp011WpfXamlExtractionStage : IExtractionStage
    {
        /// <summary>
        /// Stores the WPF extractor that performs deterministic static XAML analysis.
        /// </summary>
        private readonly WpfXamlExtractor _extractor;

        /// <summary>
        /// Stores the logger used for credential-safe WP011 WPF orchestration diagnostics.
        /// </summary>
        private readonly ILogger<Wp011WpfXamlExtractionStage> _logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="Wp011WpfXamlExtractionStage" /> class.
        /// </summary>
        /// <param name="extractor">The WPF extractor responsible for projecting static XAML artifacts into graph facts.</param>
        /// <param name="logger">The logger used for stage start, completion, cancellation, and degraded extraction diagnostics.</param>
        public Wp011WpfXamlExtractionStage(WpfXamlExtractor extractor, ILogger<Wp011WpfXamlExtractionStage> logger)
        {
        ...
        /// </summary>
        public string StageId => "wp011-wpf-xaml";

        /// <summary>
        /// Executes static WPF XAML extraction and merges the resulting snapshot into shared accumulation.
        /// </summary>
        ...
        /// <param name="cancellationToken">The cancellation token that stops WPF artifact discovery and graph projection.</param>
        /// <returns>A successful stage result when WPF extraction completes or produces non-fatal diagnostics.</returns>
        public async Task<ExtractionStageResult> ExecuteAsync(ExtractionStageContext context, CancellationToken cancellationToken)
        {
            // WP011 WPF extraction is non-blocking because absence of WPF artifacts should not prevent other extraction facts from being persisted.
            ArgumentNullException.ThrowIfNull(context);
            cancellationToken.ThrowIfCancellationRequested();

            _logger.LogInformation(
                "Starting WP011 WPF XAML extraction for run {RunId}.",
            ...
            WpfXamlExtractionResult result = await _extractor.ExtractAsync(new WpfXamlExtractionRequest(snapshotStableKey, context.ResolvedInput.RepositoryRootDirectory), cancellationToken).ConfigureAwait(false);
            UiSnapshotMergeSummary mergeSummary = context.Accumulation.MergeUiSnapshot(result.Snapshot);

            _logger.LogInformation(
                "Completed WP011 WPF XAML extraction for run {RunId}; added {NodeCount} node(s), {EdgeCount} edge(s), {EvidenceCount} evidence record(s), {WarningCount} warning(s), and {ErrorCount} error(s) after stable-key deduplication.",
                context.Run.RunId.ToString(),
