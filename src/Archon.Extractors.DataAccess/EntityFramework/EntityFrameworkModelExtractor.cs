...
namespace Archon.Extractors.DataAccess.EntityFramework
{
    ...
    public sealed class EntityFrameworkModelExtractor
    {
        ...
        {
            // EF extraction runs after LINQ to SQL extraction in the same WP009 entry path so callers receive one accumulated data-access snapshot.
        ...
        {
            // Context identity follows the WP009 project-plus-type stable key rule and captures provider hints without storing raw connection strings.
        ...
        {
            // Shared metadata keeps EF facts aligned with WP009 lower-camel metadata naming and source provenance rules.
        ...
        {
            // EF6 and EF Core use different migration base types but both are source-level schema artifacts for WP009.
            for (INamedTypeSymbol? current = typeSymbol; current is not null; current = current.BaseType)
            {
                string namespaceName = current.ContainingNamespace.ToDisplayString();
        ...
        {
            // These names cover the WP009-required providers and are extensible without changing graph contracts.
        ...
        {
            // Provider metadata uses WP009 controlled values as strings rather than new graph node kinds.
        ...
        /// <summary>
        /// Normalizes provider invariant names into stable WP009 provider values.
        /// </summary>
