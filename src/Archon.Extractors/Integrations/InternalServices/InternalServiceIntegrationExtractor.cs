...
namespace Archon.Extractors.Integrations.InternalServices
{
    ...
    public sealed class InternalServiceIntegrationExtractor
    {
        ...
        {
            // Internal correlation follows the same collect-then-project shape as other external-integration slices so quality behavior stays consistent.
        ...
        private static void AnalyzeSemanticDocument(Archon.Roslyn.SemanticModel.SemanticExtractionRequest semanticDocument, InternalServiceEndpointIndex endpointIndex, List<ExternalIntegrationObservation> observations, List<string> warnings, CancellationToken cancellationToken)
        {
            ...
            {
                warnings.Add("internal service correlation skipped a non-C# document because this slice currently supports C# syntax for client call extraction while preserving Roslyn semantic parity limits.");
                return;
            }
        ...
        private static void AnalyzeInvocation(Archon.Roslyn.SemanticModel.SemanticExtractionRequest semanticDocument, InvocationExpressionSyntax invocation, InternalServiceEndpointIndex endpointIndex, List<ExternalIntegrationObservation> observations, List<string> warnings, CancellationToken cancellationToken)
        {
            ...
            if (descriptor.UnknownReason is not null)
            {
                ...
                warnings.Add($"internal service correlation recorded an unknown target because {descriptor.UnknownReason}");
                return;
            }
            ...
            if (endpoint is null)
            {
                ...
                observations.Add(unknownObservation);
                warnings.Add($"internal service correlation recorded an unknown target because {unknownReason}");
                return;
            }
        ...
        {
            // Internal service calls still project to ExternalService nodes so existing external-integration consumers can display all outbound dependencies consistently.
        ...
        {
            // Source keys match the existing method/type key convention so cross-slice deduplication stays stable.
