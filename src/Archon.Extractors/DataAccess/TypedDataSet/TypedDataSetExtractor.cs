...
namespace Archon.Extractors.DataAccess.TypedDataSet
{
    ...
    public sealed class TypedDataSetExtractor
    {
        ...
        {
            // Typed DataSet extraction is deliberately part of the existing data-access entry path so callers receive a single accumulated data-access snapshot.
        ...
        {
            // The typed DataSet itself is represented as an Entity node because data-access uses subtyped metadata values rather than introducing additional graph node kinds.
        ...
        {
            // The first token is enough for the bounded read/write classification supported by data-access.
        ...
        {
            // Column metadata uses the same lower-camel database field names as the other data-access slices.
            Dictionary<string, object?> values = CreateBaseMetadata(relativePath, dataSetName);
            values["schemaName"] = tableName.SchemaName;
            values["tableName"] = tableName.ObjectName;
        ...
        {
            // Shared metadata keeps typed DataSet facts aligned with data-access lower-camel naming and source provenance rules.
