...
namespace Archon.Extractors.DataAccess.LinqToSql
{
    ...
    {
        /// <summary>
        /// Stores graph node kinds that participate in final data-access cross-slice correlation.
        /// </summary>
        ...
        /// <summary>
        /// Adds deterministic correlations between data-access data-access facts and graph facts emitted by earlier configuration, dependency-injection, and runtime stages.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity and semantic source documents.</param>
        /// <param name="accumulator">The shared accumulator containing all current data-access and precursor API-stage facts.</param>
        ...
        {
            // Registration edges are emitted by dependency-injection extraction before data-access runs; matching by type identity avoids depending on container implementation details.
        ...
        /// <summary>
        /// Links AddDbContext-style registration calls to known data-access context nodes when dependency-injection extraction has not emitted a generic registration edge for the extension method.
        /// </summary>
        ...
        {
            // Entity Framework registrations are provider setup calls as well as DI registrations, so data-access records the context correlation even if the general DI extractor treats the API as framework-specific.
        ...
        {
            // Metadata follows data-access lower-camel field names and excludes any raw connection string value.
        ...
        /// <summary>
        /// Creates deterministic metadata for a data-access final cross-slice correlation edge.
        /// </summary>
        ...
        /// <summary>
        /// Creates a deterministic architecture edge for final data-access cross-slice correlation.
        /// </summary>
        ...
        private static ArchitectureEdge CreateCorrelationEdge(StableKey snapshotStableKey, EdgeKind edgeKind, StableKey sourceStableKey, StableKey targetStableKey, StableKey? primaryEvidenceStableKey, GraphMetadata metadata, Confidence confidence)
        {
            ...
            StableKey stableKey = new($"data-access-correlation-edge://{HashStablePayload(edgeKind.Value, sourceStableKey.Value, targetStableKey.Value, metadata.ToCanonicalJson())}");
        ...
        {
            // Modern and legacy dependency-injection extractors use different stable-key namespaces, so matching accepts exact keys and terminal path segments only.
        ...
        /// <summary>
        /// Normalizes a provider string into a small stable data-access provider vocabulary where possible.
        /// </summary>
        ...
        /// <summary>
        /// Holds stable lookup tables used by final data-access cross-slice correlation after all extractor slices have contributed graph facts.
        /// </summary>
        private sealed class CrossSliceCorrelationState
        {
            ...
            /// <param name="dataAccessNodes">Data-access nodes emitted by data-access slices.</param>
            /// <param name="contextNodes">Data-access context nodes emitted by data-access slices.</param>
            ...
            /// <summary>
            /// Gets data-access nodes emitted by data-access slices.
            /// </summary>
            public IReadOnlyList<ArchitectureNode> DataAccessNodes { get; }

            /// <summary>
            /// Gets EF and LINQ to SQL context nodes emitted by data-access slices.
            /// </summary>
            ...
            /// <summary>
            /// Gets runtime method nodes emitted by runtime extraction, keyed by qualified method identity.
            /// </summary>
            ...
            {
                // Runtime method nodes carry runtimeKind metadata, which avoids hard-coding individual runtime stable-key formats.
