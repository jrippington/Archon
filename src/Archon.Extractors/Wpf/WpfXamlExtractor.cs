...
{
    /// <summary>
    /// Extracts static UI extraction WPF XAML facts from project files, XAML markup, and code-behind source into shared graph contracts.
    /// </summary>
    /// <remarks>
    /// The extractor performs deterministic repository-file analysis only. It does not compile WPF projects, instantiate controls, evaluate resources, run dispatchers, render XAML, connect to databases, or write directly to persistence.
    /// </remarks>
    public sealed partial class WpfXamlExtractor
    {
        /// <summary>
        /// Extracts WPF application, window, page, user-control, resource, style, template, binding, command, routed-event, navigation, view-model, service, data-access, evidence, warning, and unknown facts.
        /// </summary>
        /// <param name="request">The repository-scoped WPF extraction request.</param>
        ...
        public async Task<WpfXamlExtractionResult> ExtractAsync(WpfXamlExtractionRequest request, CancellationToken cancellationToken = default)
        {
            ...
            IReadOnlyList<WpfProjectContext> projects = DiscoverProjects(request.RepositoryRootDirectory);
            IReadOnlyList<WpfArtifactContext> artifacts = DiscoverArtifacts(request.RepositoryRootDirectory, projects);
            WpfRepositoryContext repositoryContext = await BuildRepositoryContextAsync(projects, artifacts, cancellationToken).ConfigureAwait(false);

            foreach (WpfProjectContext project in projects.Where(project => artifacts.Any(artifact => StringComparer.Ordinal.Equals(artifact.Project.RelativeProjectPath, project.RelativeProjectPath))))
            {
                cancellationToken.ThrowIfCancellationRequested();
                AccumulateProjectAndApplication(request, accumulator, project, artifacts, repositoryContext);
            }

            foreach (WpfArtifactContext artifact in artifacts.Where(artifact => artifact.ArtifactKind is WpfArtifactKind.Window or WpfArtifactKind.Page or WpfArtifactKind.UserControl or WpfArtifactKind.ResourceDictionary).OrderBy(artifact => artifact.RelativePath, StringComparer.OrdinalIgnoreCase))
            {
                cancellationToken.ThrowIfCancellationRequested();
                AnalyzeArtifact(request, accumulator, repositoryContext, artifact);
            }

            return new WpfXamlExtractionResult(accumulator.ToSnapshot());
        }

        /// <summary>
        /// Discovers WPF-capable projects from project metadata, references, application definitions, XAML files, and source symbols.
        /// </summary>
        ...
        private static IReadOnlyList<WpfProjectContext> DiscoverProjects(string repositoryRootDirectory)
        {
            ...
            List<WpfProjectContext> projects = [];
            ...
            foreach (string projectPath in projectPaths)
            {
                ...
                if (!metadata.IsWpfCandidate)
                {
                    continue;
                }

                projects.Add(new WpfProjectContext(projectPath, metadata.RelativeProjectPath, metadata.ProjectName, metadata.TargetFramework, metadata.Language, metadata.StartupObject, metadata.StartupUri, metadata.PackageIdentities));
            }
        ...
        /// <summary>
        /// Discovers XAML and source artifacts that belong to discovered WPF projects.
        /// </summary>
        /// <param name="repositoryRootDirectory">The accepted repository root directory.</param>
        /// <param name="projects">The WPF project contexts that can own artifacts.</param>
        /// <returns>Artifact contexts ordered by repository-relative path.</returns>
        private static IReadOnlyList<WpfArtifactContext> DiscoverArtifacts(string repositoryRootDirectory, IReadOnlyList<WpfProjectContext> projects)
        {
            // Discovery includes XAML and code-behind because WPF UI structure spans declarative markup and event/dependency source.
            if (!Directory.Exists(repositoryRootDirectory) || projects.Count == 0)
            {
                return [];
            }

            List<WpfArtifactContext> artifacts = [];
            ...
            {
                WpfProjectContext? project = FindNearestProject(projects, artifactPath);
                if (project is null)
                {
                    continue;
                }
                ...
                WpfArtifactKind artifactKind = ClassifyArtifact(relativePath, content);
                string typeName = ExtractPrimaryTypeName(relativePath, content, artifactKind);
                artifacts.Add(new WpfArtifactContext(project, artifactPath, relativePath, typeName, artifactKind));
            }
        ...
        /// <summary>
        /// Builds repository-wide WPF context used to correlate XAML, code-behind, view models, services, and data-access usage.
        /// </summary>
        /// <param name="projects">The discovered WPF projects.</param>
        /// <param name="artifacts">The discovered WPF artifacts.</param>
        /// <param name="cancellationToken">The cancellation token that stops source loading.</param>
        /// <returns>A repository context used while analyzing WPF artifacts.</returns>
        private static async Task<WpfRepositoryContext> BuildRepositoryContextAsync(IReadOnlyList<WpfProjectContext> projects, IReadOnlyList<WpfArtifactContext> artifacts, CancellationToken cancellationToken)
        {
            ...
            Dictionary<string, WpfArtifactContext> artifactByType = new(StringComparer.Ordinal);
            ...
            foreach (WpfArtifactContext artifact in artifacts)
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                if (artifact.ArtifactKind is WpfArtifactKind.Window or WpfArtifactKind.Page or WpfArtifactKind.UserControl or WpfArtifactKind.Application)
                {
                    artifactByType[artifact.TypeName] = artifact;
                    AddTypePath(sourcePathsByType, artifact.TypeName, artifact.RelativePath);
                }

                if (artifact.ArtifactKind is WpfArtifactKind.Code)
                {
            ...
            return new WpfRepositoryContext(sourceByPath, artifactByType, sourcePathsByType, serviceTypeNames, viewModelTypeNames, eventsByType, serviceUsagesByType, dataAccessUsagesByType);
        }

        /// <summary>
        /// Adds project and UI application facts for one WPF project.
        /// </summary>
        ...
        /// <param name="artifacts">The discovered WPF artifacts used to resolve application definitions.</param>
        /// <param name="repositoryContext">The repository context that supplies source content and startup evidence.</param>
        private static void AccumulateProjectAndApplication(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfProjectContext project, IReadOnlyList<WpfArtifactContext> artifacts, WpfRepositoryContext repositoryContext)
        {
            // Project and application facts give all WPF UI nodes stable ownership when this extractor runs independently from project inventory.
            StableKey projectStableKey = StableKeyGenerator.ForProject(project.RelativeProjectPath);
            EvidenceRecord projectEvidence = UiEvidenceFactory.CreateMarkupEvidence(request.SnapshotStableKey, new UiSourceLocation(project.RelativeProjectPath, 1, 1, project.ProjectName), "Wpf", "Application", "ProjectMetadata", Confidence.High, UnknownState.Known);
            ...
            WpfArtifactContext? applicationArtifact = artifacts.FirstOrDefault(artifact => StringComparer.Ordinal.Equals(artifact.Project.RelativeProjectPath, project.RelativeProjectPath) && artifact.ArtifactKind is WpfArtifactKind.Application);
            ...
            UnknownState unknownState = isStartupUnknown ? UnknownState.Unknown("WPF startup URI or startup object could not be resolved statically.") : UnknownState.Known;
            Confidence confidence = isStartupUnknown ? Confidence.Low : Confidence.High;
            string applicationSourcePath = applicationArtifact?.RelativePath ?? project.RelativeProjectPath;
            StableKey applicationStableKey = UiStableKeyBuilder.Create("ui-application://", projectStableKey.Value, "Wpf", project.TargetFramework, startupIdentity);
            ...
            {
                ["confidenceReason"] = isStartupUnknown ? "No static StartupUri or StartupObject value identified the WPF startup artifact." : "Project metadata or Application XAML identified the WPF startup artifact.",
            ...
            {
                accumulator.AddWarning($"WPF startup artifact for {project.RelativeProjectPath} could not be statically resolved.");
            }
        }

        /// <summary>
        /// Analyzes one WPF XAML artifact and contributes graph facts for supported markup and source patterns.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        /// <param name="repositoryContext">The repository context used for source and symbol correlation.</param>
        /// <param name="artifact">The WPF XAML artifact being analyzed.</param>
        private static void AnalyzeArtifact(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfRepositoryContext repositoryContext, WpfArtifactContext artifact)
        {
            ...
            StableKey ownerStableKey = CreateArtifactNode(request, accumulator, artifact, projectStableKey, artifactEvidence.StableKey, document.Root);

            foreach (ResourceUsage resource in ExtractResources(document, artifact))
            {
            ...
            foreach (BindingUsage binding in ExtractBindings(document, artifact))
            {
                AccumulateBinding(request, accumulator, artifact, projectStableKey, ownerStableKey, binding);
            }

            foreach (CommandUsage command in ExtractCommands(document, artifact))
            {
                AccumulateCommand(request, accumulator, artifact, projectStableKey, ownerStableKey, command);
            }

            foreach (EventUsage routedEvent in ExtractMarkupEvents(document, artifact).Concat(repositoryContext.EventsByType.TryGetValue(artifact.TypeName, out IReadOnlyList<EventUsage>? codeEvents) ? codeEvents : []))
            {
                AccumulateEvent(request, accumulator, artifact, projectStableKey, ownerStableKey, routedEvent);
            }

            foreach (NavigationUsage navigation in ExtractNavigation(document, artifact))
            {
        ...
        /// <summary>
        /// Creates the primary graph node for a WPF XAML artifact.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives the graph node.</param>
        /// <param name="artifact">The WPF artifact being represented.</param>
        ...
        private static StableKey CreateArtifactNode(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey evidenceStableKey, XElement root)
        {
            // Framework-specific subtypes remain metadata values; the node kind uses the shared static UI extraction UI vocabulary.
            NodeKind nodeKind = artifact.ArtifactKind switch
            {
                WpfArtifactKind.Window => NodeKind.UiView,
                WpfArtifactKind.Page => NodeKind.UiPage,
                WpfArtifactKind.UserControl => NodeKind.UiComponent,
                WpfArtifactKind.ResourceDictionary => NodeKind.UiResource,
                _ => NodeKind.UiComponent
            };
            string displayName = artifact.TypeName;
            ...
            if (artifact.ArtifactKind is WpfArtifactKind.Window)
            {
                metadata["windowName"] = displayName;
            }
            else if (artifact.ArtifactKind is WpfArtifactKind.Page)
            {
                metadata["pageName"] = displayName;
            }
            else if (artifact.ArtifactKind is WpfArtifactKind.ResourceDictionary)
            {
                metadata["resourceKey"] = Path.GetFileName(artifact.RelativePath);
            }

            accumulator.AddNode(CreateNode(request.SnapshotStableKey, nodeStableKey, nodeKind, displayName, artifact.RelativePath, displayName, "XAML", projectStableKey, projectStableKey, Confidence.High, UnknownState.Known, evidenceStableKey, metadata));
        ...
        /// <summary>
        /// Adds graph facts for a WPF resource, style, or template observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateResource(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, ResourceUsage resource)
        {
            ...
            StableKey resourceStableKey = UiStableKeyBuilder.Create("ui-resource://", projectStableKey.Value, "Wpf", artifact.RelativePath, resource.ArtifactKind, resource.Key, resource.LineNumber.ToString(CultureInfo.InvariantCulture));
            ...
            {
                ["confidenceReason"] = resource.IsUnknown ? resource.UnknownReason : "Static WPF resource evidence.",
            ...
            {
                accumulator.AddWarning($"WPF {resource.ArtifactKind.ToLowerInvariant()} in {artifact.RelativePath} has unresolved dynamic resource evidence: {resource.Key}.");
            }
        }

        /// <summary>
        /// Adds graph facts for a WPF control or nested component observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateControl(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, ControlUsage control)
        {
            // Named controls are represented as UiControl nodes, while nested custom controls also use UiControl until type-resolution contracts are richer.
            StableKey controlStableKey = UiStableKeyBuilder.Create("ui-control://", projectStableKey.Value, "Wpf", artifact.RelativePath, control.ControlType, control.ControlName, control.LineNumber.ToString(CultureInfo.InvariantCulture));
        ...
        /// <summary>
        /// Adds graph facts for a WPF binding observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateBinding(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, BindingUsage binding)
        {
            // Unqualified `{Binding}` expressions are explicit unknowns because their runtime target depends on DataContext shape.
            UnknownState unknownState = binding.IsUnknown ? UnknownState.Unknown("WPF binding path could not be resolved statically.") : UnknownState.Known;
            Confidence confidence = binding.IsUnknown ? Confidence.Low : Confidence.High;
            StableKey bindingStableKey = UiStableKeyBuilder.Create("ui-binding://", projectStableKey.Value, "Wpf", artifact.RelativePath, binding.PropertyName, binding.BindingPath, binding.LineNumber.ToString(CultureInfo.InvariantCulture));
            ...
            {
                accumulator.AddWarning($"WPF unresolved binding path in {artifact.RelativePath} at line {binding.LineNumber.ToString(CultureInfo.InvariantCulture)}.");
            }
        }

        /// <summary>
        /// Adds graph facts for a WPF command binding observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateCommand(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, CommandUsage command)
        {
            // Command bindings are modeled separately from routed events because they usually target view-model command properties.
            StableKey commandStableKey = UiStableKeyBuilder.Create("ui-command://", projectStableKey.Value, "Wpf", artifact.RelativePath, command.CommandName, command.LineNumber.ToString(CultureInfo.InvariantCulture));
        ...
        /// <summary>
        /// Adds graph facts for a WPF routed event handler observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateEvent(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, EventUsage eventUsage)
        {
            // Routed events are represented by command nodes so handlers can be traversed uniformly with command facts.
            StableKey commandStableKey = UiStableKeyBuilder.Create("ui-command://", projectStableKey.Value, "Wpf", artifact.RelativePath, eventUsage.HandlerName, eventUsage.LineNumber.ToString(CultureInfo.InvariantCulture));
        ...
        /// <summary>
        /// Adds graph facts for a WPF navigation observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateNavigation(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, NavigationUsage navigation)
        {
            ...
            UnknownState unknownState = navigation.IsUnknown ? UnknownState.Unknown("WPF navigation target is computed from runtime state.") : UnknownState.Known;
            Confidence confidence = navigation.IsUnknown ? Confidence.Low : Confidence.High;
            StableKey targetStableKey = UiStableKeyBuilder.Create("ui-page://", projectStableKey.Value, "Wpf", artifact.RelativePath, navigation.Target, navigation.LineNumber.ToString(CultureInfo.InvariantCulture));
            EvidenceRecord evidence = CreateEvidence(request, artifact, navigation.SourceText, "Page", "XamlNavigation", confidence, unknownState, navigation.LineNumber);
            accumulator.AddEvidence(evidence);
            ...
            {
                ["confidenceReason"] = navigation.IsUnknown ? "Navigation target uses a binding or runtime expression." : "Navigation target is a static XAML source.",
                ["detectionMode"] = "XamlNavigation",
            ...
            {
                accumulator.AddWarning($"WPF computed navigation target in {artifact.RelativePath} at line {navigation.LineNumber.ToString(CultureInfo.InvariantCulture)}.");
            }
        }

        /// <summary>
        /// Adds graph facts for WPF view-model evidence or convention-only unknowns.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        /// <param name="root">The parsed XAML root element.</param>
        private static void AccumulateViewModel(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfRepositoryContext repositoryContext, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, XElement root)
        {
            ...
            UnknownState unknownState = usage.IsUnknown ? UnknownState.Unknown("WPF view model is inferred by convention only and was not found in source.") : UnknownState.Known;
            Confidence confidence = usage.IsUnknown ? Confidence.Low : usage.Confidence;
            StableKey viewModelStableKey = UiStableKeyBuilder.Create("ui-viewmodel://", projectStableKey.Value, "Wpf", artifact.RelativePath, usage.ViewModelType, usage.LineNumber.ToString(CultureInfo.InvariantCulture));
            ...
            if (usage.IsUnknown)
            {
                accumulator.AddWarning($"WPF convention-only view model for {artifact.TypeName} in {artifact.RelativePath} could not be found in source.");
            }
        }

        /// <summary>
        /// Adds graph facts for a WPF code-behind service usage.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateServiceUsage(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, ServiceUsage serviceUsage)
        {
        ...
        /// <summary>
        /// Adds graph facts for a WPF code-behind data-access or external package usage.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        ...
        private static void AccumulateDataAccessUsage(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, DataAccessUsage dataAccessUsage)
        {
        ...
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        /// <param name="accumulator">The accumulator that receives graph facts.</param>
        /// <param name="artifact">The WPF artifact that contains runtime-dependent markup.</param>
        /// <param name="projectStableKey">The owning project stable key.</param>
        /// <param name="ownerStableKey">The stable key of the artifact containing the unknowns.</param>
        /// <param name="root">The parsed XAML root element.</param>
        private static void AccumulateDynamicUnknowns(WpfXamlExtractionRequest request, ArchitectureSnapshotAccumulator accumulator, WpfArtifactContext artifact, StableKey projectStableKey, StableKey ownerStableKey, XElement root)
        {
            ...
            foreach (UnknownUsage unknown in ExtractUnknowns(root, artifact))
            {
                UnknownState unknownState = UnknownState.Unknown(unknown.UnknownReason);
                StableKey unknownStableKey = UiStableKeyBuilder.Create("ui-unknown://", projectStableKey.Value, "Wpf", artifact.RelativePath, unknown.Category, unknown.LineNumber.ToString(CultureInfo.InvariantCulture));
                ...
                accumulator.AddWarning($"WPF {unknown.Category} in {artifact.RelativePath} requires runtime information: {unknown.UnknownReason}");
            }
        }

        /// <summary>
        /// Reads WPF-relevant metadata from a C# or VB.NET project file.
        /// </summary>
        /// <param name="repositoryRootDirectory">The accepted repository root directory.</param>
        ...
        private static ProjectMetadata ReadProjectMetadata(string repositoryRootDirectory, string projectPath)
        {
            ...
            {
                // Malformed project files cannot be evaluated safely; source evidence may still identify WPF through future enhancement, so this project is skipped for now.
                return new ProjectMetadata(relativeProjectPath, projectName, "Unknown", language, null, null, [], false);
            }
        }

        /// <summary>
        /// Classifies a repository artifact into a WPF XAML/source category.
        /// </summary>
        /// <param name="relativePath">The repository-relative artifact path.</param>
        /// <param name="content">The artifact content.</param>
        /// <returns>The artifact kind used by WPF extraction.</returns>
        private static WpfArtifactKind ClassifyArtifact(string relativePath, string content)
        {
            // Classification relies on XAML root tags and source naming because extractor execution must not load WPF assemblies.
            ...
            {
                return WpfArtifactKind.Code;
            }

            string trimmed = content.TrimStart();
            if (trimmed.StartsWith("<Application", StringComparison.OrdinalIgnoreCase))
            {
                return WpfArtifactKind.Application;
            }

            if (trimmed.StartsWith("<Window", StringComparison.OrdinalIgnoreCase))
            {
                return WpfArtifactKind.Window;
            }

            if (trimmed.StartsWith("<Page", StringComparison.OrdinalIgnoreCase))
            {
                return WpfArtifactKind.Page;
            }

            if (trimmed.StartsWith("<UserControl", StringComparison.OrdinalIgnoreCase))
            {
                return WpfArtifactKind.UserControl;
            }

            if (trimmed.StartsWith("<ResourceDictionary", StringComparison.OrdinalIgnoreCase))
            {
                return WpfArtifactKind.ResourceDictionary;
            }

            return WpfArtifactKind.Other;
        }

        /// <summary>
        /// Extracts the primary type or artifact name from WPF markup or source content.
        /// </summary>
        /// <param name="relativePath">The repository-relative artifact path.</param>
        /// <param name="content">The artifact content.</param>
        /// <param name="artifactKind">The artifact kind being named.</param>
        /// <returns>The primary artifact type name.</returns>
        private static string ExtractPrimaryTypeName(string relativePath, string content, WpfArtifactKind artifactKind)
        {
            // XAML `x:Class` is authoritative; code artifacts use source declarations; resource dictionaries fall back to file names.
            if (artifactKind is WpfArtifactKind.Code)
            {
        ...
        private static XDocument? TryLoadXaml(string content, string relativePath, ArchitectureSnapshotAccumulator accumulator)
        {
            ...
            {
                accumulator.AddWarning($"WPF XAML artifact {relativePath} could not be parsed: {exception.Message}");
                return null;
            }
        }

        /// <summary>
        /// Extracts resource, style, and template observations from a parsed WPF XAML document.
        /// </summary>
        ...
        private static IReadOnlyList<ResourceUsage> ExtractResources(XDocument document, WpfArtifactContext artifact)
        {
        ...
        /// <summary>
        /// Extracts named controls and custom component usages from a parsed WPF XAML document.
        /// </summary>
        /// <param name="document">The parsed XAML document.</param>
        /// <param name="artifact">The artifact that owns the document.</param>
        /// <returns>Control observations in document order.</returns>
        private static IReadOnlyList<ControlUsage> ExtractControls(XDocument document, WpfArtifactContext artifact)
        {
        ...
        /// <summary>
        /// Extracts binding expressions from attributes in a parsed WPF XAML document.
        /// </summary>
        /// <param name="document">The parsed XAML document.</param>
        /// <param name="artifact">The artifact that owns the document.</param>
        /// <returns>Binding observations in document order.</returns>
        private static IReadOnlyList<BindingUsage> ExtractBindings(XDocument document, WpfArtifactContext artifact)
        {
            // WPF binding markup extensions can contain `Path=Name`, direct `Binding Name`, or bare `{Binding}` expressions.
            List<BindingUsage> bindings = [];
            foreach (XElement element in document.Descendants())
            {
                foreach (XAttribute attribute in element.Attributes())
                {
                    if (!attribute.Value.Contains("{Binding", StringComparison.Ordinal))
                    {
                        continue;
                    }
        ...
        /// <summary>
        /// Extracts command bindings from WPF Command attributes.
        /// </summary>
        /// <param name="document">The parsed XAML document.</param>
        /// <param name="artifact">The artifact that owns the document.</param>
        /// <returns>Command observations in document order.</returns>
        private static IReadOnlyList<CommandUsage> ExtractCommands(XDocument document, WpfArtifactContext artifact)
        {
        ...
        /// <summary>
        /// Extracts routed event attributes from WPF XAML markup.
        /// </summary>
        /// <param name="document">The parsed XAML document.</param>
        /// <param name="artifact">The artifact that owns the document.</param>
        /// <returns>Event observations in document order.</returns>
        private static IReadOnlyList<EventUsage> ExtractMarkupEvents(XDocument document, WpfArtifactContext artifact)
        {
            // Routed events are inferred from known WPF event attribute names with method-like handler values.
            List<EventUsage> events = [];
            foreach (XElement element in document.Descendants())
            {
                foreach (XAttribute attribute in element.Attributes().Where(attribute => IsRoutedEventAttribute(attribute)))
                {
        ...
        /// <summary>
        /// Extracts navigation targets from WPF frame and hyperlink source attributes.
        /// </summary>
        /// <param name="document">The parsed XAML document.</param>
        /// <param name="artifact">The artifact that owns the document.</param>
        /// <returns>Navigation observations in document order.</returns>
        private static IReadOnlyList<NavigationUsage> ExtractNavigation(XDocument document, WpfArtifactContext artifact)
        {
            // Frame Source and NavigateUri values are static navigation targets unless they contain markup extensions or bindings.
            ...
            foreach (XElement element in document.Descendants())
            {
                foreach (XAttribute attribute in element.Attributes().Where(attribute => string.Equals(attribute.Name.LocalName, "Source", StringComparison.Ordinal) || string.Equals(attribute.Name.LocalName, "NavigateUri", StringComparison.Ordinal)))
                {
                    ...
                    navigations.Add(new NavigationUsage(attribute.Value.Trim(), isUnknown, GetLineNumber(element), element.ToString(SaveOptions.DisableFormatting)));
                }
            }
        ...
        /// <summary>
        /// Extracts a direct or convention-based view-model usage for a WPF artifact.
        /// </summary>
        /// <param name="root">The parsed XAML root element.</param>
        /// <param name="artifact">The artifact being analyzed.</param>
        /// <param name="repositoryContext">The repository context containing known view-model types.</param>
        /// <returns>The view-model usage classification.</returns>
        private static ViewModelUsage ExtractViewModel(XElement root, WpfArtifactContext artifact, WpfRepositoryContext repositoryContext)
        {
        ...
        /// <summary>
        /// Extracts explicit unknown runtime-dependent WPF observations from parsed markup.
        /// </summary>
        /// <param name="root">The parsed XAML root element.</param>
        /// <param name="artifact">The artifact that owns the markup.</param>
        /// <returns>Unknown observations in document order.</returns>
        private static IReadOnlyList<UnknownUsage> ExtractUnknowns(XElement root, WpfArtifactContext artifact)
        {
            // Unknown extraction focuses on runtime patterns called out by static UI extraction rather than attempting to model every WPF dynamic feature.
            ...
            foreach (XElement element in root.DescendantsAndSelf())
            {
                foreach (XAttribute attribute in element.Attributes())
                {
                    ...
                    {
                        unknowns.Add(new UnknownUsage("dynamic resource", "DynamicResource", "Resource", NodeKind.UiResource, "DynamicResource", "WPF dynamic resource target is computed from runtime state.", GetLineNumber(element), element.ToString(SaveOptions.DisableFormatting)));
                    }
                    ...
                    {
                        unknowns.Add(new UnknownUsage("runtime template", "RuntimeTemplate", "Style", NodeKind.UiStyle, "RuntimeTemplate", "WPF template selection is determined at runtime.", GetLineNumber(element), element.ToString(SaveOptions.DisableFormatting)));
                    }
                }
            }
        ...
        /// <summary>
        /// Extracts repository-local service type declarations.
        /// </summary>
        /// <param name="content">The source content.</param>
        /// <returns>Service type names in source order.</returns>
        private static IEnumerable<string> ExtractRepositoryServiceTypeNames(string content)
        {
            // Repository service declarations allow UI usage nodes to line up with existing code symbols where available.
        ...
        /// <summary>
        /// Resolves WPF startup identity from project metadata and application XAML.
        /// </summary>
        /// <param name="project">The project whose startup identity is being resolved.</param>
        /// <param name="applicationArtifact">The optional application XAML artifact.</param>
        /// <param name="repositoryContext">The repository context containing application XAML content.</param>
        /// <returns>The startup identity when statically available; otherwise, Unknown.</returns>
        private static string ResolveStartupIdentity(WpfProjectContext project, WpfArtifactContext? applicationArtifact, WpfRepositoryContext repositoryContext)
        {
            // StartupUri in App.xaml is preferred because it identifies the actual UI artifact used by WPF at runtime.
            if (applicationArtifact is not null && repositoryContext.SourceByPath.TryGetValue(applicationArtifact.RelativePath, out string? content))
            {
                Match match = StartupUriRegex().Match(content);
                if (match.Success)
                {
                    return match.Groups["uri"].Value.Trim();
                }
            }
            ...
            return !string.IsNullOrWhiteSpace(project.StartupUri) ? project.StartupUri : "Unknown";
        }

        /// <summary>
        /// Creates a source-backed evidence record for one WPF observation.
        /// </summary>
        /// <param name="request">The extraction request that scopes snapshot identity.</param>
        ...
        private static EvidenceRecord CreateEvidence(WpfXamlExtractionRequest request, WpfArtifactContext artifact, string sourceText, string artifactKind, string detectionMode, Confidence confidence, UnknownState unknownState, int? lineNumber = null)
        {
            ...
            int startLine = lineNumber ?? FindLineNumber(sourceText, sourceText);
            return UiEvidenceFactory.CreateMarkupEvidence(request.SnapshotStableKey, new UiSourceLocation(artifact.RelativePath, startLine, startLine, sourceText), "Wpf", artifactKind, detectionMode, confidence, unknownState);
        }
        ...
        /// <summary>
        /// Creates a stable key for a WPF primary artifact node.
        /// </summary>
        /// <param name="artifact">The artifact being represented.</param>
        /// <param name="projectStableKey">The owning project stable key.</param>
        /// <returns>The deterministic artifact stable key.</returns>
        private static StableKey CreateArtifactStableKey(WpfArtifactContext artifact, StableKey projectStableKey)
        {
            // Primary artifact identity uses project key, framework, repository-relative path, artifact kind, and type name.
            return UiStableKeyBuilder.Create("ui-artifact://", projectStableKey.Value, "Wpf", artifact.RelativePath, artifact.ArtifactKind.ToString(), artifact.TypeName);
        }
        ...
        /// <summary>
        /// Extracts a WPF binding path from a markup extension value.
        /// </summary>
        ...
        {
            // The parser handles common WPF binding forms without attempting to evaluate the full markup-extension grammar.
        ...
        /// <summary>
        /// Determines whether an element is a root WPF artifact element.
        /// </summary>
        ...
        private static bool IsRootArtifactElement(XElement element, WpfArtifactContext artifact)
        {
            // Root elements are already represented by the artifact node and should not also become child controls.
            return element.Parent is null && artifact.ArtifactKind is WpfArtifactKind.Window or WpfArtifactKind.Page or WpfArtifactKind.UserControl;
        }

        /// <summary>
        /// Determines whether an element is a project-local custom component reference.
        /// </summary>
        /// <param name="element">The candidate element.</param>
        ...
        {
            // WPF CLR namespace mappings appear as non-standard XML namespaces and identify project or assembly component references.
            return element.Name.NamespaceName.StartsWith("clr-namespace:", StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            // Any keyed object under WPF resources can be referenced by StaticResource or DynamicResource.
        ...
        {
            // The current slice recognizes common routed events and requires method-like handler names to avoid classifying regular text properties as events.
            string name = attribute.Name.LocalName;
            return (name is "Click" or "Loaded" or "SelectionChanged" or "MouseDown" or "MouseUp" or "KeyDown" or "KeyUp")
        ...
        {
            // Multiple partial files can contribute to one WPF type, so each type maps to a set of source paths.
        ...
        private static WpfProjectContext? FindNearestProject(IReadOnlyList<WpfProjectContext> projects, string artifactPath)
        {
        ...
        /// <summary>
        /// Gets the metadata artifact-kind value for a WPF artifact kind.
        /// </summary>
        /// <param name="artifactKind">The WPF artifact kind.</param>
        /// <returns>The UI artifact-kind metadata value.</returns>
        private static string GetArtifactKindMetadata(WpfArtifactKind artifactKind)
        {
            // Metadata uses shared static UI extraction artifact names rather than WPF-specific graph node kinds.
            return artifactKind switch
            {
                WpfArtifactKind.Application => "Application",
                WpfArtifactKind.Window => "View",
                WpfArtifactKind.Page => "Page",
                WpfArtifactKind.UserControl => "Component",
                WpfArtifactKind.ResourceDictionary => "Resource",
                WpfArtifactKind.Code => "CodeBehind",
                _ => "Unknown"
            };
        }
        ...
        /// <summary>
        /// Creates a regex for WPF StartupUri attributes.
        /// </summary>
        /// <returns>A regex that captures the startup URI.</returns>
        [GeneratedRegex("StartupUri=\"(?<uri>[^\"]+)\"", RegexOptions.CultureInvariant)]
        private static partial Regex StartupUriRegex();

        /// <summary>
        /// Creates a regex for C# or VB class declarations.
        /// </summary>
        /// <returns>A regex that captures class names.</returns>
        [GeneratedRegex("\\b(?:public|internal|private|partial|sealed|static|NotInheritable|Partial|Class|Friend|Public|Private|Protected|\\s)*class\\s+(?<name>[A-Za-z_][A-Za-z0-9_]*)\\b|\\bClass\\s+(?<name>[A-Za-z_][A-Za-z0-9_]*)\\b", RegexOptions.CultureInvariant | RegexOptions.IgnoreCase)]
        ...
        /// <summary>
        /// Creates a regex for WPF code-behind routed event handler methods.
        /// </summary>
        ...
        /// <summary>
        /// Creates a regex for direct `{Binding Name}` syntax.
        /// </summary>
        /// <returns>A regex that captures binding paths.</returns>
        ...
        /// <summary>
        /// Describes one discovered WPF-capable project.
        /// </summary>
        ...
        /// <param name="StartupUri">The optional application definition include value read from project metadata.</param>
        /// <param name="PackageIdentities">The package identities declared by the project.</param>
        private sealed record WpfProjectContext(string AbsoluteProjectPath, string RelativeProjectPath, string ProjectName, string TargetFramework, string Language, string? StartupObject, string? StartupUri, IReadOnlyList<string> PackageIdentities);
        ...
        /// <param name="StartupUri">The optional startup URI or application definition path.</param>
        /// <param name="PackageIdentities">The package identities declared by the project.</param>
        /// <param name="IsWpfCandidate">Whether the project contains WPF evidence.</param>
        private sealed record ProjectMetadata(string RelativeProjectPath, string ProjectName, string TargetFramework, string Language, string? StartupObject, string? StartupUri, IReadOnlyList<string> PackageIdentities, bool IsWpfCandidate);

        /// <summary>
        /// Describes one discovered WPF artifact and its owning project.
        /// </summary>
        ...
        /// <param name="ArtifactKind">The coarse WPF artifact classification.</param>
        private sealed record WpfArtifactContext(WpfProjectContext Project, string AbsolutePath, string RelativePath, string TypeName, WpfArtifactKind ArtifactKind);

        /// <summary>
        /// Describes repository-wide WPF context used during per-artifact analysis.
        /// </summary>
        ...
        private sealed record WpfRepositoryContext(IReadOnlyDictionary<string, string> SourceByPath, IReadOnlyDictionary<string, WpfArtifactContext> ArtifactByType, IReadOnlyDictionary<string, HashSet<string>> SourcePathsByType, IReadOnlySet<string> ServiceTypeNames, IReadOnlySet<string> ViewModelTypeNames, IReadOnlyDictionary<string, IReadOnlyList<EventUsage>> EventsByType, IReadOnlyDictionary<string, IReadOnlyList<ServiceUsage>> ServiceUsagesByType, IReadOnlyDictionary<string, IReadOnlyList<DataAccessUsage>> DataAccessUsagesByType);
        ...
        /// <summary>
        /// Describes a WPF resource, style, or template observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes a WPF control observation.
        /// </summary>
        /// <param name="ControlName">The control name or type when unnamed.</param>
        /// <param name="ControlType">The WPF control type.</param>
        ...
        /// <summary>
        /// Describes a WPF binding observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes a WPF command binding observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes a WPF routed event observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes a WPF navigation observation.
        /// </summary>
        ...
        /// <param name="LineNumber">The one-based source line number.</param>
        /// <param name="SourceText">The source text used for evidence.</param>
        private sealed record NavigationUsage(string Target, bool IsUnknown, int LineNumber, string SourceText);

        /// <summary>
        /// Describes a WPF view-model correlation observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes a WPF runtime-dependent unknown observation.
        /// </summary>
        ...
        /// <summary>
        /// Describes the coarse category of a WPF artifact.
        /// </summary>
        private enum WpfArtifactKind
        {
            /// <summary>
            /// A WPF application definition XAML file.
            /// </summary>
            Application,

            /// <summary>
            /// A WPF window XAML file.
            /// </summary>
            Window,

            /// <summary>
            /// A WPF page XAML file.
            /// </summary>
            Page,

            /// <summary>
            /// A WPF user-control XAML file.
            /// </summary>
            UserControl,

            /// <summary>
            /// A WPF resource dictionary XAML file.
            /// </summary>
            ResourceDictionary,
