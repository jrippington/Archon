...
- Roslyn and extractor projects, which provide analysis and source-evidence extraction areas. The current Roslyn slice places language-neutral semantic contracts in `src/Archon.Roslyn` and compiler-backed C# and VB.NET extraction support in the language-specific Roslyn projects. The extractor estate is now represented by the solution-owned `src/Archon.Extractors` production project and `test/Archon.Extractors.Tests` test project. Extractor categories are folders and namespaces inside those consolidated assemblies, such as `src/Archon.Extractors/DataAccess` with namespace `Archon.Extractors.DataAccess` and `test/Archon.Extractors.Tests/DataAccess` with namespace `Archon.Extractors.Tests.DataAccess`. The old fine-grained category project directories have been retired after migration, so new extractor work should extend the consolidated projects rather than adding another `Archon.Extractors.<Category>` assembly.
...
The consolidated extractor production project follows the same Onion rule as the former extractor category projects: it can depend inward on application contracts, domain contracts, and Roslyn helper projects, but it must not depend on infrastructure adapters, API hosts, the Aspire AppHost, or test projects. The consolidated extractor test project exists as the matching test assembly for the complete extractor test estate. It references `src/Archon.Extractors` and carries the shared xUnit, coverage, Roslyn C#, and Roslyn Visual Basic package baseline used by the migrated tests. Category separation is now represented by folders and namespaces below `Archon.Extractors` and `Archon.Extractors.Tests`, rather than by fine-grained assemblies. Marker types still identify their conceptual categories through namespaces and type names, but the physical assembly identity for migrated production categories is `Archon.Extractors`. Contributors should add implementation files to the appropriate category folder, add tests to the corresponding consolidated test category, and avoid creating new `Archon.Extractors.<Category>` or `Archon.Extractors.<Category>.Tests` projects unless a later architecture decision explicitly reopens that pattern. The retired category project directories have been deleted; their absence is now part of the expected repository shape rather than a missing-project warning.
