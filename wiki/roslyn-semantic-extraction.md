...
The shared contracts live in `src/Archon.Roslyn`, which is the language-neutral Roslyn project. That project defines semantic requests, results, declaration facts, relationship facts, source language values, symbol identity, evidence, and deterministic helpers. The C# implementation lives in `src/Archon.Roslyn.CSharp`, and the VB.NET implementation lives in `src/Archon.Roslyn.VisualBasic`; both depend inward on the shared Roslyn abstractions. The infrastructure Roslyn adapter in `src/Archon.Infrastructure.Roslyn` now provides the first API-orchestrated loading and projection path for real repository fixtures. It reads explicitly submitted solution files, loads supported C# and VB.NET projects, parses source documents, creates Roslyn compilations, invokes the language-specific extractors, and projects semantic results into the shared application snapshot accumulator.

The WP007 dependency-injection extractor also consumes Roslyn semantic document context, but it is a specialized extractor rather than a replacement for the generic semantic declaration and relationship slice. The generic Roslyn slice answers broad questions such as “which methods call which methods?” and “which constructor parameters are injected?” The DI slice asks a narrower runtime-composition question: “which implementation type did a Microsoft DI registration bind to which service abstraction?” It walks invocation syntax, uses the semantic model to prove the registration method and generic type arguments, and then emits domain graph contracts described in [configuration and dependency-injection extraction](configuration-and-dependency-injection-extraction.md).

This boundary preserves the Onion Architecture rule. Roslyn language projects perform compiler-backed extraction and produce graph-ready facts, but they do not write Neo4j data and do not own API route behavior. The infrastructure Roslyn stage is responsible for repository file loading and graph projection, while the API module only composes the stage into the application pipeline. Neo4j persistence remains a separate infrastructure adapter that receives generic `ArchitectureNode`, `ArchitectureEdge`, and `EvidenceRecord` contracts. Keeping extraction graph-ready rather than persistence-bound lets the same semantic facts support API workflows, MCP workflows, markdown export, and future analysis without duplicating extraction logic.

## API orchestration and snapshot projection

Semantic extraction is now invoked from the shared API extraction orchestration path. After the `project-repository-solution` stage contributes repository, solution, project, package, and file artifact facts, the `roslyn-semantic` stage analyzes the same explicit submitted solution boundary. This sequencing matters because semantic source facts need a stable repository root and project context, and the project extraction stage has already established repository-relative project identities that semantic projection can reuse.

The semantic stage projects declarations and relationships into snapshot-scoped graph contracts. A semantic declaration fact becomes an `ArchitectureNode` with the Roslyn stable key as its graph stable key, a node kind such as `Namespace`, `Type`, `Method`, `Property`, or `Field`, a project stable key derived from the repository-relative project file path, source language, confidence, and primary compiler-symbol evidence. A semantic relationship fact becomes an `ArchitectureEdge` with a shared edge kind such as `CONTAINS`, `CALLS`, `IMPLEMENTS`, `INHERITS`, `INJECTS`, or `DEPENDS_ON`, source and target stable keys, confidence, unknown state where needed, and primary evidence. Compiler diagnostics become compiler-diagnostic evidence and non-blocking warnings. Explicit unknown facts become inference evidence and non-blocking warnings so the run records what could not be proven without preventing resolvable facts from reaching persistence.

This projection means semantic facts reach the same application-layer `IArchitectureSnapshotWriter` port as earlier project and package facts. The Neo4j adapter does not know about Roslyn syntax trees or semantic models; it simply maps the generic graph contracts it receives. Repeated extraction of unchanged semantic inputs produces deterministic node, edge, and evidence stable keys when the accepted run and source inputs are the same.
...
These commands validate shared helper behavior, degraded semantic contracts, C# declaration and relationship extraction behavior, C# diagnostics and unknown handling, VB.NET declaration and relationship extraction behavior, VB.NET diagnostics and unknown handling, legacy generated-code classification, semantic infrastructure loading and projection, API orchestration of semantic extraction, generic Neo4j mapper behavior for semantic graph facts, and integrated solution compilation. They intentionally do not start the Aspire AppHost. The Neo4j mapper command does not require Neo4j credentials because it validates contract-to-parameter mapping rather than opening a database connection.
