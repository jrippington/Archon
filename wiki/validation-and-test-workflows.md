...
Before running this command, make sure Docker Desktop or another OCI-compatible runtime is available. In the Aspire dashboard, confirm that `neo4j`, `ArchonApi`, `ArchonMcp`, and `ArchonExplorer` appear and that no `ArchonUi` or Discovery UI resource appears. Open the `ArchonExplorer` resource URL and confirm that the browser renders the workbench shell. Stop the AppHost manually after verification.

## ArchonExplorer frontend foundation and shell validation
...
Run these commands from the repository root when changing the frontend skeleton, provider setup, TypeScript configuration, Vite configuration, package manifest, package lockfile, shadcn-compatible component metadata, shared UI primitives, workbench shell components, local workbench state, layout preference persistence, activity navigation, primary sidebar placeholders, bottom-panel behavior, command palette behavior, shell command registration, keyboard shortcut handling, notification host placement, status-bar slots, tabbed work area behavior, ArchonApi route catalog, API request executor, typed operational API client, query-key helpers, extraction polling helpers, runtime test doubles, connectivity-state helpers, connectivity hook, notification runtime, safe error shaping helpers, typed API contracts, theme tokens, or frontend CSS:
...
`npm install` maintains the deterministic `package-lock.json` file, so it should be run after package version changes and in clean environments. `npm run test` runs the Vitest unit suite, including route-catalog tests that prove static route constants, path-value encoding, grouped route exports, and the no-common-`/api` convention; request/error tests that prove safe JSON parsing, empty response handling, missing base URL behavior, validation-problem shaping, safe query error shaping, redaction of unsafe text, network failure classification, timeout/cancellation classification, malformed JSON handling, and unexpected content-type handling; operational-client/connectivity tests that prove typed client route usage, destructive-operation no-retry intent, safe connectivity-state transitions, and safe status-bar text; query-key/polling/test-double tests that prove stable server-state cache identity, invalidation selector targets, bounded extraction polling intervals, terminal stop conditions, cancellation, stalled-operation handling, route-catalog-backed mock behavior, typed mock contracts, and destructive delete-all confirmation behavior; workbench shell tests that prove initial shell rendering, local activity selection, sidebar context updates, default tab state, tab fallback recovery, layout preference defaults, preference save/load/reset behavior, invalid storage recovery, bottom-panel visibility, safe bottom-panel placeholders, command registration, command-palette grouping, command execution through local state, disabled future-search feedback, shortcut recognition, notification-host placement, status-bar slot text, layout size bounds, and safe placeholder text; and notification-runtime tests that prove category handling, normalized-error conversion, unsafe-detail suppression, accessible rendering structure, and provider composition. `npm run typecheck` is the fast static check for TypeScript source and configuration. `npm run build` runs the same TypeScript project references and then builds the Vite output. The generated `dist` directory is a local build artifact and should not be treated as source guidance.

Focused browser validation for the current WP003 shell uses Playwright from the same frontend project directory:

```powershell
cd .\src\ArchonExplorer
npm install
npx playwright install chromium
npm run test:e2e -- workbench-shell
```

Playwright is the browser automation runner for end-to-end shell journeys. The frontend-local `playwright.config.ts` starts the Vite development server on a local port, keeps reports and test results beside `src/ArchonExplorer`, and runs the focused shell suite from `src/test-e2e`. The `workbench-shell` journey opens the shell, verifies accessible landmarks for the activity rail, primary sidebar, tabbed work area, status bar, command affordance, and notification host, opens the command palette with the documented keyboard shortcut, confirms focus moves into the filter input, switches activities without leaving the frame, toggles the bottom panel through visible controls and shell commands, checks resize separator labels and values, and confirms status text communicates state in words rather than color alone. A clean machine may need `npx playwright install chromium` once after package restore so the browser binary exists. This command is focused browser validation for the shell; it does not start ArchonApi, Neo4j, the MCP host, or the Aspire AppHost.
...
When iterating only on the WP003 workbench shell state, activity navigation, sidebar placeholders, tabbed work area, layout preference persistence, bottom panel, resizable layout seams, command palette, shell commands, keyboard shortcut handling, notification host placement, or status-bar shell slots, this targeted command is useful before the full frontend validation sequence:
...
When iterating only on the Snapshot Workspace extraction submission, history, selected-run detail, duplicate-request, or produced-snapshot placeholder slices, this targeted command is useful before the full frontend validation sequence. Some package scripts and file paths still use `extraction-center` because those names predate the product-level Snapshot Workspace terminology, but the browser journey now opens the Snapshot Workspace workbench context rather than a separate Extraction Center page:
...
The targeted command exercises the extraction history hook, start-extraction request mapping, duplicate-request reconstruction from selected run status, metadata-value omission guidance, compact-history unavailable duplication guidance, typed API client usage, extraction history and selected-run query keys, deterministic client test double, polling state mapping, background tracking state, terminal acknowledgement rules, bottom-panel monitor rendering, Snapshot Workspace command registration, safe normalized error mapping, empty history rendering, populated history rendering, accepted-run summary rendering, selected-run detail rendering, persistence diagnostic rendering, produced-snapshot placeholder rendering, selectable history rows, API-unconfigured feedback, validation problem mapping, and unsafe diagnostic suppression. It does not start ArchonApi or prove live backend availability; the route and response behavior remain covered by API tests and focused browser mocks.
...
When the browser-visible change affects Snapshot Workspace submission, update status, history, selected-run detail, background monitoring, duplicate-request, or produced-snapshot handoff behavior, pair the shell journey with the focused Snapshot Workspace Playwright journey:
...
The manual smoke check should confirm that the page identifies itself as `ArchonExplorer`, shows the app frame, activity rail, command/search affordance, primary sidebar, tabbed work area with the `Workbench Start` tab, controllable bottom panel, command palette, notification host, status bar, theme control, and safe API connectivity indicator. Select several activities, such as Snapshots, Search, and Diagnostics, and confirm that the primary sidebar changes context while the tabbed work area remains inside the same shell frame. Press `Ctrl+K` or activate `Open command palette`, confirm that focus moves into the palette, run a shell command such as `Toggle Bottom Panel` or `Reset Layout Preferences`, and confirm that state changes stay inside the workbench frame. Inspect the future architecture search command and confirm that it explains the later-work-package boundary without displaying search results, graph records, raw Cypher, or architecture artefacts. Show and hide the bottom panel, resize layout regions with the available separator controls, reload the page, and confirm that safe layout preferences persist or recover to defaults when browser storage is unavailable or reset. With no API base URL, the status bar should explain that the API base URL is not configured. With a configured but unavailable API, it should report unreachable status with safe text. With a healthy and ready API, it should report that the API is reachable and ready. Stop the Vite process after inspection. This manual check is not a substitute for `npm run typecheck` and `npm run build`; it is the practical browser-facing confirmation that the shell can be served by Vite and that unavailable states remain visibly honest.
...
## WP019 persisted run history and snapshot management validation

WP019 validates the point where operational extraction history and persisted snapshot lifecycle management meet. A contributor should read this section with [API extraction workflow](api-extraction-workflow.md) and [Neo4j persistence foundation](neo4j-persistence-foundation.md). The important mental model is that an extraction run is a durable operational record, while a snapshot is the persisted architecture knowledge record that a completed run may produce. A run can fail without a snapshot, and a completed run can retain its public snapshot stable key after the snapshot-scoped graph has been deleted for local cleanup. Validation should therefore prove both sides of the boundary: run history remains readable, and destructive cleanup removes only the snapshot-owned graph records it is supposed to remove.

Use focused application and API tests first when changing management validation, request/response contracts, route metadata, safe error shaping, or in-memory fallback behavior:

```powershell
dotnet test .\test\Archon.Application.Tests\Archon.Application.Tests.csproj --filter "FullyQualifiedName~StartExtractionApplicationServiceTests|FullyQualifiedName~ExtractionOrchestratorTests|FullyQualifiedName~ManagementOperationsServiceTests"
dotnet test .\test\Archon.Api.Extraction.Tests\Archon.Api.Extraction.Tests.csproj --filter FullyQualifiedName~ExtractionEndpointTests
dotnet test .\test\Archon.Api.Management.Tests\Archon.Api.Management.Tests.csproj --filter FullyQualifiedName~ManagementEndpointTests
```

These tests prove that accepted extraction requests create readable run state before scheduling, that status and recent-history routes return safe public DTOs, that management delete-one and delete-all validation happens before a destructive store operation, and that unsupported delete-all inputs such as `dryRun` or repository/date/status filters are rejected instead of being treated as future behavior. They also prove the API boundary does not expose raw Cypher, Neo4j internal IDs, stack traces, connection strings, driver exception text, or request metadata values.

Use the Neo4j infrastructure test project when changing schema names, schema statements, Cypher, graph mapping, transaction boundaries, durable run-history storage, lifecycle listing, snapshot deletion, or production composition:

```powershell
dotnet test .\test\Archon.Infrastructure.Neo4j.Tests\Archon.Infrastructure.Neo4j.Tests.csproj --filter "FullyQualifiedName~ExtractionRunHistory|FullyQualifiedName~SnapshotLifecycle|FullyQualifiedName~SnapshotDeletion|FullyQualifiedName~Neo4jInfrastructureComposition|FullyQualifiedName~Neo4jServiceCollectionExtensions"
dotnet test .\test\ArchonApi.Tests\ArchonApi.Tests.csproj --filter FullyQualifiedName~ArchonApi
dotnet build .\Archon.slnx --no-restore
```

The Neo4j tests use Testcontainers rather than the Aspire AppHost. They should prove that `ArchonExtractionRun` and `ArchonExtractionRunRequest` records are written with public stable identities, that completed runs can link to existing snapshots through `PRODUCED_SNAPSHOT`, that recent history is deterministic, that lifecycle listing reads `ArchonSnapshot` headers through the application port, and that deletion removes snapshot-scoped subgraphs while preserving shared repository, solution, rule, extraction run, and extraction request records. Composition tests should prove that a Neo4j-composed host resolves `IArchitectureSnapshotWriter`, `IExtractionRunHistory`, `ISnapshotLifecycleQuery`, and `ISnapshotDeletionStore` to Neo4j-backed implementations. In-memory stores remain valid only as explicit test or lightweight local fallbacks.

Manual cleanup verification is intentionally a local-development workflow. To verify delete-one, seed or produce one snapshot in a disposable local Neo4j database, call `DELETE /management/snapshots/{snapshotStableKey}` with the URL-encoded public snapshot stable key, then list snapshots and poll the producing run. The expected result is that the lifecycle row and snapshot-scoped graph records disappear, while the run remains readable with its historical snapshot stable key. To verify delete-all, seed or produce multiple snapshots, call `POST /management/snapshots/delete-all` with a JSON body containing `confirmation: "delete-all-snapshots"`, then list snapshots and poll recent run history. The expected result is zero lifecycle rows and no snapshot-scoped graph records, with run history and shared records still present. Do not use these management cleanup operations against shared, production, or otherwise valuable data unless an explicit operational policy exists for that environment.
