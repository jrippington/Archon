...
A `USES_PACKAGE` edge between a project node and a package node means the source project file, or a repository-contained imported build file, declared an SDK-style `PackageReference`. Package node identity is based on normalized NuGet package ID and known version or explicit version-source state. For example, a direct `PackageReference Include="Serilog" Version="4.0.0"` becomes a package identity such as `package://serilog/version/4.0.0`. If a package has no direct version and no local central version, the package node keeps an unknown version-source identity rather than pretending the version is absent by design. The `USES_PACKAGE` edge metadata preserves version source, directness, asset metadata, import state, and evidence path so contributors can distinguish direct project-file package declarations from imported build-file declarations.
