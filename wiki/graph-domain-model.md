...
The current project extraction pipeline uses this accumulator as the shared contribution model. Pipeline stages receive the accumulator, add warnings, errors, or graph facts, and return a stage result that tells the runner whether later stages can safely continue. The first project extraction stage contributes repository and solution architecture nodes, repository-to-solution `CONTAINS` edges, and evidence records for submitted solution files. Snapshot assembly then merges those accumulated contributions with deterministic repository and solution boundary models from the accepted request. This means the assembled snapshot contains both the boundary model sections and queryable graph-node/edge/evidence sections for the same repository and submitted solutions. Empty finding, metric, and generated-summary collections are explicit current-state output, not missing serialization fields.

For repository and solution extraction, evidence-first modeling is visible immediately. A solution node points to file-level solution evidence, while each visible solution `Project(...) = ...` declaration can produce line-level evidence that later project metadata slices can reuse. The current slice intentionally stops short of creating project nodes from those declarations. That separation keeps the first extraction capability small and deterministic while preserving enough source explanation for later solution-to-project membership facts.
