...
Project inventory facts are the current graph representation of submitted solution membership, core project-file metadata, and project-to-project dependencies. A project inventory fact starts with a supported C# or VB.NET project declaration in a submitted solution file. The declaration gives Archon a project display name, a declared path, a solution project type GUID, and a source line that can be recorded as evidence. The extractor then reads the project file as XML and contributes a `Project` architecture node with a repository-relative stable key.
...
The current project extraction pipeline uses this accumulator as the shared contribution model. Pipeline stages receive the accumulator, add warnings, errors, or graph facts, and return a stage result that tells the runner whether later stages can safely continue. The current project extraction stage contributes repository, solution, and project architecture nodes; repository-to-solution and solution-to-project `CONTAINS` edges; project-to-project `REFERENCES` edges; evidence records for submitted solution files; line-level evidence for solution project declarations; file-level evidence for supported project files; and project-reference evidence for explicit `ProjectReference` items. Snapshot assembly then merges those accumulated contributions with deterministic repository and solution boundary models from the accepted request. This means the assembled snapshot contains both the boundary model sections and queryable graph-node/edge/evidence sections for the same repository, submitted solutions, supported project files, and repository-contained project dependency targets. Empty finding, metric, and generated-summary collections are explicit current-state output, not missing serialization fields.

For repository, solution, and project extraction, evidence-first modeling is visible immediately. A solution node points to file-level solution evidence, each visible solution `Project(...) = ...` declaration can produce line-level evidence, a supported project node points to file-level project evidence, and a project-reference relationship points to the `ProjectReference` item that declared it. Unsupported declarations and unresolved references remain visible as warnings and evidence instead of disappearing from the run, while a run with no supported C# or VB.NET project declarations fails with a controlled blocking error so contributors know the submitted solution set did not produce extractable project inventory facts.
