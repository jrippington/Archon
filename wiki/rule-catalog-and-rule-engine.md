...
The extraction workflow now contains a rule catalog evaluation stage with the stable identifier `wp012-rule-catalog-evaluation`. It runs after the project, semantic, configuration, runtime, data-access, external-integration, and UI/client stages have had an opportunity to contribute graph facts to the shared accumulator. This ordering matters because rule evaluation is intentionally a classifier over already-extracted graph facts. The stable identifier is still useful for pipeline timings and compatibility, while runtime log messages describe the behavior as rule catalog evaluation so operators see what the stage is doing rather than which delivery work package introduced it. The rule stage does not load project files independently, does not run Roslyn independently, does not start the target application, and does not query Neo4j for arbitrary graph data before evaluating. It receives the same accumulated snapshot shape that later persistence receives and projects the fields needed by the evaluator into the smaller `RuleEvaluationGraph` read model.
