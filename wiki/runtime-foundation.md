...
Archon's runtime foundation is deliberately small. The API host and MCP host can start as ASP.NET Core processes, expose operational probes, and run under an Aspire AppHost for local development. The API host now also composes the asynchronous [API extraction workflow](api-extraction-workflow.md), which maps start, status, and recent-history endpoints while keeping extraction validation, run lifecycle behavior, progress updates, orchestration, snapshot assembly, and persistence handoff in application services. That workflow executes the current project extraction stage for explicitly submitted solution files, so repository, solution, project, package, analyzer, FilePath, evidence, warning, and application type classification facts can flow into a generalized snapshot. The runtime foundation still does not implement query APIs, MCP graph tools, markdown export, deeper Roslyn symbol extraction, user-interface behavior, or arbitrary build execution.
