...
4. [Roslyn semantic extraction](roslyn-semantic-extraction.md) explains the current compiler-backed C# declaration and relationship extraction slice, semantic stable keys, evidence, confidence, and dependency facts.
5. [Neo4j persistence foundation](neo4j-persistence-foundation.md) explains schema initialization, guarded recreation, snapshot persistence, relationship nodes, and persisted analysis outputs.
6. [API extraction workflow](api-extraction-workflow.md) explains `POST /extractions`, status polling, validation boundaries, run lifecycle state, and the initial scheduler/run-history seams.
