...
1. Before making Aspire runtime or AppHost changes, run the apphost using `aspire run` when that validation is relevant and practical, then inspect the state of resources to make sure you are building from a known state. Do not use this recommendation as a reason to stop or wait during an active work item; if the apphost is unnecessary for the current change, continue with the next required task.
